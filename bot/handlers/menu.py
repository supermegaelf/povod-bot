...
from bot.utils.messaging import safe_answer_callback, send_or_edit
...
async def show_community(callback: CallbackQuery) -> None:
    ...
    await send_or_edit(callback, text, reply_markup=back_to_main_keyboard(), is_edit=True, disable_web_page_preview=True)
    await safe_answer_callback(callback)
...
async def show_settings(callback: CallbackQuery) -> None:
    ...
    await send_or_edit(callback, t("moderator.settings_title"), reply_markup=keyboard, is_edit=True)
    await safe_answer_callback(callback)
